...
// Battle keeps today's {0, 0.5, 1, 2} ratios at the corrected absolute scale: this is
// PROVISIONAL. The original's TACP binary shares the cityscape's pacer and its
// 36-per-second accumulator, but static
...
// Legacy speeds: before this branch, battle delivered a
...
{
	// The legacy option selects the whole tier table, not per-tier - see the
...
// Delivery stays chunked at today's granularity: a big single-call jump tunnels
...
void BattleView::applyUpdateSpeed(BattleUpdateSpeed newSpeed)
{
	...
	{
		// A paused view must accrue nothing; zero any fraction left over from
...
void BattleView::update(const StageFrame &frame)
{
	...
	// hideDisplay runs its own fixed-rate accumulator; otherwise the normal
