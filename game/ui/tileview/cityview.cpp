...
// Legacy speeds: before this branch, OpenApoc delivered
...
{
	// The legacy option picks the tier table AND the turbo rate together, never
...
// Delivery stays chunked at today's granularity: big single-call jumps tunnel
      ...
      followVehicle(false), selectionState(CitySelectionState::Normal)
{
	...
	this->baseForm->findControl("BUTTON_SPEED0")
	    ->addCallback(FormEventType::CheckBoxSelected,
	                  [this](Event *) { applyUpdateSpeed(CityUpdateSpeed::Pause); });
	this->baseForm->findControl("BUTTON_SPEED1")
	    ->addCallback(FormEventType::CheckBoxSelected,
	                  [this](Event *) { applyUpdateSpeed(CityUpdateSpeed::Speed1); });
	this->baseForm->findControl("BUTTON_SPEED2")
	    ->addCallback(FormEventType::CheckBoxSelected,
	                  [this](Event *) { applyUpdateSpeed(CityUpdateSpeed::Speed2); });
	this->baseForm->findControl("BUTTON_SPEED3")
	    ->addCallback(FormEventType::CheckBoxSelected,
	                  [this](Event *) { applyUpdateSpeed(CityUpdateSpeed::Speed3); });
	this->baseForm->findControl("BUTTON_SPEED4")
	    ->addCallback(FormEventType::CheckBoxSelected,
	                  [this](Event *) { applyUpdateSpeed(CityUpdateSpeed::Speed4); });
	this->baseForm->findControl("BUTTON_SPEED5")
	    ->addCallback(FormEventType::CheckBoxSelected,
	                  [this](Event *) { applyUpdateSpeed(CityUpdateSpeed::Speed5); });
...
void CityView::applyUpdateSpeed(CityUpdateSpeed newSpeed)
{
	this->updateSpeed = newSpeed;
	auto rate = cityTickRate(newSpeed);
	tickAccumulator.setRate(rate.numerator, rate.denominator);
	tickAccumulator.setMaxTicksPerAdvance(
	    TickAccumulator::ticksInOneClamp(rate.numerator, rate.denominator, STAGE_FRAME_CLAMP_US));
	if (newSpeed == CityUpdateSpeed::Pause)
	{
		// A covered/paused view must accrue nothing; zero any fraction left over
