...
// Changing TargetFPS must change smoothness only - the delivered rate (and so
...
// City Speed1..Speed4 must match the original's measured rates to tighter than 1%.
...
// zero the fractional remainder when it trips: otherwise one stall becomes a
...
// Pause (and Stage::resume()) must zero the fractional remainder so a stale
...
// updates the top stage); the first post-resume frame must then behave like an
...
static bool test_covered_view_then_resume_is_one_ordinary_frame()
{
	...
	// On resume(), the remainder is zeroed and the next frame is fed normally.
	covered.reset();
...
// ticksInOneClamp is the suggested per-frame ceiling default (the ticks one clamp-width
// of real time produces at the current speed); check it against hand-computed values
// for the city and battle tiers this branch actually ships.
...
// the corrected city/turbo rates the option defaults to, duplicated from
