...
/*
    ...
    same unit tests:
      ...
      - `maxTicksPerAdvance` bounds a single advance() call's *output*. Ticks beyond the
        ceiling are discarded, not carried, and the fractional remainder is zeroed.
        Carrying a backlog forward instead would turn one stall into a sustained
        catch-up that keeps re-tripping the ceiling on every subsequent frame.
*/
class TickAccumulator
{
  public:
	...
	// Stage::resume(): it stops a stale fraction accrued at a previous speed from
	...
	// Ticks produced by one elapsedClampUs-wide burst at the given rate: this is the
